...
;if(0)fprintf(stderr, "%d %d %d\n", (int) n_thread_l, (int) n_thread_l, (int) n_group_G)
   ...
   ;  else
      ...
      ;  }
         ...
         mclxVectorDispatchGroup(mx, data, n_thread_l, diam_dispatch, n_group_G, i_group)
      ;  mcxFree(data)
      ;  mclxFree(&scratchen)
   ;  }
...
,  mclv*       lattice_order
   ...
   ;  compute_scores_up2(down, up, lattice_order, n_order)

   ;  for (j=1;j<n_order;j++)  /* j =0 is an endpoint */
      {  dim k = lattice_order->ivps[j].idx
      ;  if (mclvSum(down->cols+k) > 0.5)
...
,  dim thread_id
   ...
   ;  ctty_compute2(mx, i, d->ctty, d->down, d->up, d->lattice_order)
;  }
...
,  const char*  argv_unused[]    cpl__unused
   ...
   ;  if (n_group_G * n_thread_l <= 1)
      ...
      ;  for (i=0; i<N_COLS(mx); i++)
         ctty_compute2(mx, i, ctty->cols+0, down, up, lattice_order)
   ;  }
      else
      {  dim t = 0
      ;  ctty_data* data = mcxAlloc(n_thread_l * sizeof data[0], EXIT_ON_FAIL)
      ;  for (t=0; t<n_thread_l; t++)
         {  ctty_data* d  = data+t
         ;  d->ctty  =  ctty->cols+t
         ;  d->down  =  mclxCopy(mx)
      ...
      ;  }
         mclxVectorDispatchGroup(mx, data, n_thread_l, ctty_dispatch, n_group_G, i_group)
