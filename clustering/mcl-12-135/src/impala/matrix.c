...
(  mclx* mx
,  const mclv* coldom
,  const mclv* _restrict
,  mcxenum scratch_STATUS
,  mclv* dst
)
   {  return mclgUnionv2(mx, coldom, _restrict, scratch_STATUS, dst, mx->dom_rows)
;  }
   ...
   ;  for (d=0;d<coldom->n_ivps;d++)
      ...
      ;  if ((dvec = mclxGetVector(mx, idx, RETURN_ON_FAIL, dvec)))
         ...
         ;  for (t=0; t<dvec->n_ivps; t++)
            ...
            ;  if
               (  _restrict
               && 0 > (o_restrict = mclvGetIvpOffset(_restrict, idx, o_restrict))
               )
