...
SAMTOOLS=$PWD/external/samtools
...
then
	...
	then
	    ...
	    qsub -sync yes -b y -pe smp 4 -v SUBJECT=$1,QUERY1=$2,QUERY2="",OUTPUT=$3 $CMD
    	trap - INT TERM EXIT
    elif [ $# -eq 4 ]
    then
    	...
    	TARGET=( ${BASE}.sam ${BASE}.bam ${BASE}.bai ${BASE}.idxstats ${BASE}.flagstats )
        trap 'rollback_rm_files ${TARGET[@]}; exit $?' INT TERM EXIT
        qsub -sync yes -b y -pe smp 4 -v SUBJECT=$1,QUERY1=$2,QUERY2=$3,OUTPUT=$4 $CMD
    	trap - INT TERM EXIT
    fi
	echo "Finished"
