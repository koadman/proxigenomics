...
def approx_intra_sim(v, g):
    card_v = g.degree(v, weight='weight') + 1.
    if card_v <= 1:
...
def relative_density(v, g):
    ...
    density_v = len([u for u in v_star if g.degree(u, weight='weight') <= deg_v])
...
class Cover:
    ...
    def shared(self, v):
        ...
        return set([ui for ui in v_adj if self.hubs.degree(ui, weight='weight') > 1])
    ...
    def write_mcl(self, path, no_isolates=False):
        ...
        with open(path, 'w') as out:
            ...
            hub_dict = dict((v, self.hubs.degree(v, weight='weight')) for v in self.hubs
                            if self.hubs.node[v]['hub'] and self.hubs.degree(v, weight='weight') >= min_deg)
...
def ocluster(g, debug=False):
    ...
    C.hubs.add_nodes_from(set(v for v in g.nodes_iter() if g.degree(v, weight='weight') == 0), hub=True, isolate=True)
    ...
    deg_dict = dict((vi, g.degree(vi, weight='weight')) for vi in C.hubs if C.hubs.node[vi]['hub'])  # hubs only
...
if __name__ == '__main__':
    ...
    if args.format == 'graphml':
        ...
        with open(args.output[0], 'w') as out:
            ...
            # build a list of hubs and their degree, sort be descending degree for output
            hubs = cover.hubs
            hub_dict = dict((v, hubs.degree(v, weight='weight')) for v in hubs if hubs.node[v]['hub'] and hubs.degree(v, weight='weight') >= min_deg)
            desc_deg = sorted(hub_dict, key=hub_dict.get, reverse=True)
            for vi in desc_deg:
