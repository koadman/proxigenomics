...
sub qfilter_trim_paired {
	...
	my $trim_cmd = "java -jar $DIR/trimmomatic.jar SE -threads $threads $trim_phred $WD/$p1_name.both.fastq $WD/$p1_name.trim.fastq ILLUMINACLIP:$adapter:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:36";
...
sub get_insert($$$$) {
	...
	my $cmdout = `java -jar $DIR/$cmd 2> /dev/null`;
	chomp $cmdout;
...
sub finalize {
	...
	print "[a5] Raw coverage: $x_cov\n";
	open(STATS, ">$OUTBASE.assembly_stats.csv");
	print STATS "File Name\tContigs\tScaffolds\tGenome Size\tLongest Scaffold\tN50\tRaw reads\tEC Reads\t% reads passing EC\tRaw nt\tEC nt\t% nt passing EC\tRaw cov\tMedian cov\t10th percentile cov\tbases >= Q40\tassembler version\n";
	print STATS "$OUTBASE\t$contig_num\t".@lens."\t$total\t$lens[0]\t$lens[$i]\t$total_reads\t$EC_reads\t$ec_reads_pct\t$total_bases\t$EC_bases\t$ec_bases_pct\t$x_cov\t$stats->{median_cov}\t$stats->{cov_10}\t$stats->{above_q40}\tA5-miseq $VERSION\n";
	close STATS;
	if($agp){
